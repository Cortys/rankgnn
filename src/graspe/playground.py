...
ds_train, ds_val, ds_test = provider.get_split((in_enc, out_enc), config=dict(batch_size_limit=100))
# ds_train = provider.get((in_enc, out_enc))
# ds_val, ds_test = ds_train, ds_train
opt = keras.optimizers.Adam(0.0001)

model.compile(optimizer=opt, loss="binary_crossentropy", metrics=["binary_accuracy"])
model.fit(ds_train, validation_data=ds_val, epochs=200, verbose=2)
model.evaluate(ds_test)
model.predict(ds_test), provider.get_test_split()[1]
